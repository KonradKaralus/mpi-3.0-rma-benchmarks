...
| **interaction** | Measure performance influence of tasks on target and RMA operations on each other. Contains the following configurations: 
| throughput | Configuration for measuring the data and operation throughput with different target tasks. |
| scale_dgemm | Measure the performance impact of RMA accesses on a compute bound task on the target. |
| intel_prog | Configuration for measuring the performance of artificial MPI progress with IntelMPI and the stream benchmark. |
